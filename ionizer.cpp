...
// Output files:
// clus.out  clusters of e-h pairs along the track
	...
	if( pke < 0.9 * pkeprev ) { // update
	  ...
	  for( unsigned j = 1; j <= nume; ++j ) {
	    ...
	    if( E[j] < 11.9 && Q1 < qmin )
	      ...
	      sig[1][j] = E[j] * dfdE[j] * log( Q1 / qmin ); // longitudinal excitation, Eq. (46) in Fano; Eq. (2.9) in RMP

	    double epbe = 1 - betasq * ep[1][j]; // Fano Eq. (47)
	    if( epbe < 1e-20 ) epbe = 1E-20;

	    double sgg = E[j] * dfdE[j] * (-0.5) * log( epbe*epbe + pow( betasq * ep[2][j], 2 ) );
	    ...
	    double sgh = E[j]*E[j] * ( betasq - ep[1][j] / ( pow( ep[1][j], 2 ) + pow( ep[2][j], 2 ) ) ) * thet;
	    sgh = 0.0092456 * sgh;
	    sig[3][j] = sgg + sgh;
	  ...
	  if( npm == 4 ) {  // ELECTRONS
	    ...
	    xlel = min( 2232.0 * radl * pow( pmom*pmom / (getot*zi), 2 ), 10.0*radl ); // units ?
	  }
